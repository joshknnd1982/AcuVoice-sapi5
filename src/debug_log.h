...
// Diagnostic log, on by default.
//
// Written to %LOCALAPPDATA%\AcuVoice SAPI5\acuvoice.log, which is writable without
...
//   HKCU\Software\AcuVoice SAPI5  DWORD  Logging = 0
...
inline bool BuildLogPath(wchar_t* path, size_t size, const wchar_t* suffix)
{
    ...
    if (swprintf_s(dir, L"%s\\AcuVoice SAPI5", base) < 0) {
        return false;
    }
    CreateDirectoryW(dir, nullptr);
    return swprintf_s(path, size, L"%s\\acuvoice%s.log", dir, suffix) >= 0;
}
...
inline bool Enabled()
{
    ...
    if (cached < 0) {
        cached = 1;
        HKEY key = nullptr;
        if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\AcuVoice SAPI5", 0, KEY_READ, &key)
            == ERROR_SUCCESS) {
            DWORD value = 1;
